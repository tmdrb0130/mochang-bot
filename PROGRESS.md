# PROGRESS — 2026-09-11 저녁 (대기 시간 2차: 스트리밍 미리보기 + 계측 완성) — **배포 완료**

> 이 절이 최신. 배포 2회 더(16:32 계측·상한 / 17:0x 스트리밍). 앞 절(같은 날 오전)의 Chroma 전환 위에 얹은 것.
> 실호출: 스모크 3회(전부 `X-Mochang-Test`). 서비스 DB 무관.

## 한 것 ① — 인테이크 진행 단계 표시

학생은 2분 넘게 "앞에 N명" 밖에 못 봤다. 실행 중인 코드가 자기 단계를 알리게 했다 —
`jobs.set_phase()` + `contextvars` 로 워커가 현재 작업을 들고 있어 **호출부에 인자를 넘길 필요가 없다.**
폴링 응답(`GET /jobs/{id}`)에 실려 나가므로 **요청이 한 건도 안 는다.** 한·영·중·일 4개 문구.

## 한 것 ② — 조사 단계별 계측 → "25~35초 공백" 이 풀렸다

`queries_ms` · `collect_ms` · `extract_facts_ms` · `store_open_ms` · `store_query_ms` · `intake_phase`(조사 vs 카드).
배포 직후 첫 실측:

```
인테이크 65.1초
├─ 조사 25.0초 = 검색어 1.9 + 검색·fetch·색인 9.9 + 사실추출 13.2
└─ 카드 40.1초
```

**25.0 = 1.9 + 9.9 + 13.2 로 정확히 맞는다** — 어제 말한 "정체 불명 25~35초" 는 색인 70초가
조사 구간에 섞여 있던 것이었고, 오전에 그걸 빼면서 사라졌다.

## 한 것 ③ — `store_query_ms` 4.6초의 정체는 Ollama 콜드 로드였다

가설(순차 조회)은 **틀렸다** — `query_vector_store` 는 이미 `asyncio.gather` 로 병렬이었다.
진짜 원인은 Ollama 기본값이 **유휴 5분 뒤 모델 해제**라 거의 매 조사가 콜드 로드를 문 것이다(실측 3.67초 vs 웜 0.13초, 28배).

`OllamaEmbedding(keep_alive="1h")` — **요청마다 붙는 값이라 공유 Ollama(11434, bustartup 과 같이 씀)를 재시작하지 않는다.**
bge-m3 는 VRAM 0.66GB 뿐이라 상주 부담이 없다.

## 한 것 ④ — 시간당 인테이크 상한 160 → 300 + 회계 버그

1000 은 피했다. GPU 가 아니라 **외부 쿼터** 때문이다 — 인테이크 1건이 LLM 8회 + 공공데이터·네이버 검색을 쓰는데,
1000건/시간이면 **data.go.kr 개발계정 일 1,000회 한도가 한 시간에 소진**된다. 인증 없는 공개 사이트라 이 상한이 사실상 유일한 남용 방어선이기도 하다.
(참고: 이 상한에 걸린 적은 **지금까지 0건**. 실사용 최대 시간당 21건.)

함께 고친 것: `_check_intake_rate` 가 `q.submit` **앞에서** 카운터를 올려, 제출이 거절되면 그 칸이 날아갔다.
이제 되돌린다(`_undo_intake_rate`). 단 시간당 상한 자체에 걸린 경우(`scope="ip"`)는 애초에 안 올렸으므로 건드리지 않는다 —
거기서 또 pop 하면 **남의 칸을 지운다.**

## 한 것 ⑤ — 카드 스트리밍: 첫 카드부터 보여준다

카드 생성 40초는 단일 LLM 호출이라 계산을 못 줄인다. **버려지는 출력도 없다**(summary·question·why·label·hint·source 전부 화면에 쓰인다).
그래서 계산 대신 **도착 순서**를 바꿨다 — 출력은 한 글자도 안 바뀌므로 **품질 영향 0**이다.

| | |
|---|---|
| `client.complete(on_delta=)` | 주면 스트리밍. `}` 가 올 때만 콜백(받는 쪽이 매번 JSON 을 훑는다). 콜백 예외는 삼킨다 — 미리보기 때문에 본 생성이 실패하면 안 된다 |
| `intake._complete_objects()` | 잘린 JSON 에서 **닫힌 객체만** 꺼낸다. 중괄호 깊이 추적 + 문자열 안 중괄호·이스케이프 건너뜀 |
| `intake.partial_view()` | known 슬롯·보기 2개 미만 제외. **채워 넣기(_fallback_card)·장수 제한은 하지 않는다** — 그건 출력이 다 온 뒤 판단할 일이고, 미리 하면 곧 바뀔 카드를 보여주게 된다 |
| `jobs.set_partial()` | 폴링 응답의 `partial` 로. 추가 요청 0 |
| `App.jsx` | 카드가 오면 즉시 카드 화면. **답 비우기를 시작 시점으로 옮겼다** — 예전처럼 결과 뒤에 비우면 미리보기에서 받은 답이 지워진다 |

**실측(실제 vLLM)**: 첫 카드 **24.7초** (전체 62.2초) — 학생 체감 **62초 → 25초**.

> 예상(8초)보다 길었다. 출력 스키마가 `summary → slots(9개) → cards` 라 첫 카드 전에 슬롯 9개를 다 뱉기 때문이다.
> 카드 생성 시작(11.6초)부터 첫 카드까지 13초가 그 preamble 이다.

## 검증

- `pytest -q` **494 passed** (489 → 494, 스트리밍 테스트 5건 추가)
- 스트리밍 테스트는 **한 글자씩 도착하는 상황**을 흉내내 1장 → 2장 순서로 알리는지 본다
- 문자열 안의 중괄호(`"가격이 {싸다}"`)에 안 속는지도 회귀로 고정
- 라이브 `ok:true` · `llm_reachable:true` · nginx 새 번들 `index-BKe7A6C5.js`

## 사고 없이 넘어간 것 (기록)

관리자 창이 `C:\WINDOWS\system32` 에서 시작돼 `deploy_window.py` 와 `cd frontend` 가 실패하고
**`nssm restart` 만 실행**됐다. 다행히 마지막 학생 요청이 22분 전이라 잃은 작업은 없다.
`npx vite build` 가 system32 에서 vite 설치를 물었고 중단했다.
→ **관리자 창 명령에는 항상 `cd C:\Users\bon505\Desktop\mochang-bot` 를 먼저 붙일 것.**

## 다음 순서

1. **외부 감시 등록** — 여전히 1순위.
2. **첫 카드를 더 당길 수 있다(결정 필요)**: `slots[].known` 의 **내용 문장은 화면·코드 어디에도 안 쓰인다**
   (`status == "known"` 만 본다). 빼면 preamble 이 줄어 첫 카드가 8~10초 당겨진다.
   **다만 위험이 있다** — `known` 을 적게 하는 것이 모델의 판단 근거(생각을 적어두는 자리) 역할을 할 수 있어,
   빼면 슬롯 판정이 나빠질 수 있다. 품질이 우선이라면 그대로 두는 것이 맞다. 중간안: `known` 을 20자 이내로 제한.
3. 조사 25초의 다음 표적은 **사실 추출 13.2초**(6회 병렬)와 **검색·fetch 9.9초**.
4. 원본 `.vectorstore/`(362MB) 정리 — 며칠 뒤.
5. `tests/test_pipeline.py.before-fix` 잔재, 테스트 초안 정리(`drafts_delete.py --tests --yes --backup`).
