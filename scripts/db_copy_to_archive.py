"""서비스 DB 를 전체기록 DB 로 통째로 복사한다 (2026-09-03, 전체기록 DB 를 처음 만들 때 한 번).

    .venv/Scripts/python scripts/db_copy_to_archive.py            # 미리보기 (경로·건수만)
    .venv/Scripts/python scripts/db_copy_to_archive.py --yes      # 복사 (백업 파일이 이미 있으면 덮어쓴다)
...
def main() -> int:
    ...
    src, dst = _path(st.url), _path(st.archive.url)
    with sqlite3.connect(src) as c:
        n = c.execute("select count(*) from drafts").fetchone()[0]
    print(f"서비스 DB {src} (초안 {n}건) → 전체기록 DB {dst}{' (있음, 덮어씀)' if dst.exists() else ''}")
    if not args.yes:
