"""테스트 초안 삭제 도구 (2026-09-03). **is_test=1 표시가 있는 초안만** 지운다 — 실사용 초안은 id 를 콕 집어도, 어떤 옵션으로도 지워지지 않는다.
    ...
    .venv/Scripts/python scripts/drafts_delete.py --list --archive       # 전체기록 DB 목록
    ...
    .venv/Scripts/python scripts/drafts_delete.py 72d5e24c --mark-test --yes --archive   # 전체기록 DB 에도 같은 표시(기록은 남기고 삭제는 안 한다 — 전례)

테스트 표시는 요청 헤더 X-Mochang-Test(사이트 주소 ?test=1, scripts/load_test.py, scripts/foreign_e2e.py)로 들어온 초안에만 붙고,
그런 초안은 서비스 DB 에 아예 들어오지 않는다(전체기록 DB 에만). 따라서 보통은 --archive 과 함께 쓴다.
...
def main() -> int:
    ...
    ap.add_argument("--archive", action="store_true", help="서비스 DB 대신 전체기록 DB(mochang-archive.sqlite)를 다룬다")
    ...
    ap.add_argument("--mark-test", action="store_true",
                    help="지정한 초안에 테스트 표시(is_test=1)를 찍는다. 운영자가 사이트에서 직접 만들어 본 초안을 지우려면 "
                         ...
                         "이 도구는 한 번에 한 DB 만 다루므로 전체기록 DB 에도 남기려면 --archive 를 붙여 한 번 더 실행한다")
    args = ap.parse_args()

    st = S.Storage.from_config(load_config())
    if args.archive:
        if st.archive is None:
            print("config.yaml storage.archive_url 이 없습니다."); return 1
        st = st.archive
    st.archive = None                     # 한 DB 만 다룬다 (미러링 없음)
    st.init()
    rows = sorted(st.list_drafts(limit=10**9), key=lambda r: r['created_at'])
    print(f"{'전체기록' if args.archive else '서비스'} DB {st.url} — 초안 {len(rows)}건 (테스트 표시 {sum(1 for r in rows if r['is_test'])}건)")
    ...
    if args.mark_test:
        ...
        where = "전체기록" if args.archive else "서비스"
        print(f"{where} DB 에 테스트 표시 {n}건. 이제 --mark-test 를 빼고 --yes 로 지울 수 있습니다.")
        if not args.archive:
            print("  (전체기록 DB 에도 표시를 남기려면: 같은 명령에 --archive 를 붙여 한 번 더 — 전체기록은 지우지 말고 표시만 두는 것이 전례다)")
        st.close()
        return 0
