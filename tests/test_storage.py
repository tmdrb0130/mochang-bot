...
def test_archive_gets_everything_and_service_skips_test_requests(tmp_path):
...
def test_archive_failure_does_not_break_service_writes(tmp_path):
    st, bak = _pair(tmp_path)
...
def test_from_config_reads_archive_url_and_ignores_same_file(monkeypatch):
    monkeypatch.delenv("MOCHANG_DATABASE_URL", raising=False)
    monkeypatch.delenv("MOCHANG_ARCHIVE_DATABASE_URL", raising=False)
    st = S.Storage.from_config({"storage": {"url": "sqlite:///a.sqlite", "archive_url": "sqlite:///b.sqlite"}})
    assert st.archive is not None and st.archive.url == "sqlite:///b.sqlite" and st.archive.archive is None
    assert S.Storage.from_config({"storage": {"url": "sqlite:///a.sqlite", "archive_url": "sqlite:///a.sqlite"}}).archive is None
    monkeypatch.setenv("MOCHANG_ARCHIVE_DATABASE_URL", "")
    assert S.Storage.from_config({"storage": {"url": "sqlite:///a.sqlite", "archive_url": "sqlite:///b.sqlite"}}).archive is None
...
def test_test_flag_only_on_archive_and_delete_refuses_real_rows(tmp_path):
    st, bak = _pair(tmp_path)
